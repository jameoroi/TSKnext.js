...
export function ReportDashboard({ initialRange, initialSales, initialAnalytics, initialError = '' }: Props) {
  ...
  const cards = [
    ...
    },
    {
      label: 'Page Views',
      value: fmt(pageviews),
      delta: change(pageviews, previousTraffic.pageviews),
      icon: Eye,
      color: '#7c3aed',
      series: trafficDaily.map((row) => row.pageviews),
    },
    {
      label: 'สมาชิกทั้งหมด',
      ...
      note: `สมัครใหม่ ${fmt(members.joined)} คน`,
    },
    {
      label: 'คำสั่งซื้อ',
    ...
    },
    {
      label: 'AOV',
      value: money(aov),
      delta: change(aov, priorAov),
      icon: TrendingUp,
      color: '#f43f5e',
      series: salesDaily.map((row) => (num(row.orders) > 0 ? num(row.revenue) / num(row.orders) : 0)),
    },
  ];
  ...
  function exportCsv() {
    ...
    section(
      'สินค้าขายดี',
      ['อันดับ', 'สินค้า', 'ชิ้น', 'ยอดขาย', 'กำไร'],
      topSold.map((row: Row, index: number) => [
        index + 1,
    ...
    section(
      'หน้าที่เข้าชมบ่อย',
      ['หน้า', 'ครั้ง'],
      topPages.map((row: Row) => [
        row.path || row.url || row.page || '/',
        row.views ?? row.count ?? row.pageviews,
      ]),
    );
    ...
    <div className="space-y-5">
      <section className="rounded-2xl border bg-white p-4 shadow-sm">
        <div className="flex flex-wrap items-center gap-2">
          <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-50 px-3 py-2 text-xs font-black text-emerald-800">
            ● LIVE ANALYTICS
          </span>
          <span
            className={`inline-flex items-center gap-1.5 rounded-full px-3 py-2 text-xs font-black ${salesSource === 'relational' ? 'bg-emerald-100 text-emerald-900' : 'bg-amber-100 text-amber-900'}`}
          >
            {salesSource === 'relational' ? 'PostgreSQL relational' : 'Commerce compatibility'}
          </span>
          <span className="inline-flex items-center gap-2 rounded-xl border px-3 py-2 text-xs font-bold text-slate-600">
          ...
          <button
            ...
            className={`rounded-xl border px-3 py-2 text-xs font-bold ${custom ? 'bg-emerald-950 text-white' : ''}`}
          >
            กำหนดเอง
          </button>
          <button
            ...
            disabled={busy}
            onClick={() => void load()}
            className="ml-auto inline-flex items-center gap-2 rounded-xl border px-3 py-2 text-xs font-bold"
          >
          ...
          <button
            type="button"
            onClick={exportCsv}
            className="inline-flex items-center gap-2 rounded-xl bg-emerald-950 px-3 py-2 text-xs font-bold text-white"
          >
            <Download size={14} />
            CSV
          </button>
        ...
        {custom && (
          <form
            ...
            className="mt-4 grid gap-3 border-t pt-4 sm:grid-cols-[1fr_1fr_auto]"
          >
            <label className="text-xs font-bold text-slate-600">
              ...
              <input
                ...
                className="mt-1 block h-11 w-full rounded-xl border px-3 text-sm"
              />
            </label>
            <label className="text-xs font-bold text-slate-600">
              ถึง
            ...
            <button
              type="submit"
              disabled={busy}
              className="self-end rounded-xl bg-emerald-950 px-4 py-3 text-sm font-bold text-white"
            >
              ใช้ช่วงนี้
            </button>
          </form>
      ...
      <div className="grid auto-cols-[minmax(190px,1fr)] grid-flow-col gap-3 overflow-x-auto pb-1 xl:grid-flow-row xl:grid-cols-6">
        {cards.map((card) => (
          <article
            key={card.label}
          ...
          >
            <div className="flex items-start justify-between gap-2">
              ...
              {card.delta != null && (
                <span
                  className={`inline-flex items-center gap-1 rounded-full px-2 py-1 text-[10px] font-black ${card.delta >= 0 ? 'bg-emerald-50 text-emerald-700' : 'bg-rose-50 text-rose-700'}`}
                >
                  {card.delta >= 0 ? <TrendingUp size={11} /> : <TrendingDown size={11} />}{' '}
                  {Math.abs(card.delta).toFixed(1)}%
                </span>
              )}
            </div>
            <p className="mt-4 text-xs font-bold text-slate-500">{card.label}</p>
            <strong className="mt-1 block text-2xl font-black text-slate-950">{card.value}</strong>
            <p className="mt-1 min-h-4 text-[10px] text-slate-400">
              {card.note || (card.delta == null ? 'ยังไม่มีช่วงก่อนหน้าให้เทียบ' : 'เทียบช่วงก่อนหน้า')}
            </p>
      ...
      <div className="grid items-stretch gap-4 xl:grid-cols-[minmax(0,1.65fr)_minmax(300px,.9fr)]">
      ...
      <div className="grid gap-5 xl:grid-cols-[1.2fr_.8fr]">
        <section className="rounded-2xl border bg-white p-5 shadow-sm">
          <div className="mb-4">
            <p className="text-xs font-black uppercase tracking-wider text-emerald-700">Customer journey</p>
            <h2 className="mt-1 text-lg font-black">Conversion Funnel</h2>
          </div>
          <div className="grid gap-2 sm:grid-cols-5">
            {stages.map(([label, value], index) => (
              <div key={label} className="rounded-2xl bg-slate-50 p-3 text-center">
                ...
                <strong className="mt-2 block text-xl">{fmt(value)}</strong>
                <small className="mt-1 block text-emerald-700">
                  {num(visitors) > 0 ? `${((num(value) / num(visitors)) * 100).toFixed(1)}%` : '—'}
                </small>
              </div>
            ))}
          </div>
        ...
        <section className="rounded-2xl border bg-white p-5 shadow-sm">
          <p className="text-xs font-black uppercase tracking-wider text-emerald-700">Profitability</p>
          <h2 className="mt-1 text-lg font-black">ยอดขายสุทธิและกำไร</h2>
      ...
      <div className="grid gap-5 xl:grid-cols-3">
        <Ranking
          ...
          icon={ShoppingCart}
          rows={topSold}
          value={(row) => `${fmt(row.units)} ชิ้น · ${money(row.revenue)}`}
        />
        <Ranking
          title="สินค้าที่ถูกดูมากที่สุด"
          icon={Package}
          rows={topViewed}
          value={(row) => `${fmt(row.views ?? row.count ?? row.value)} ครั้ง`}
        />
        <Ranking
          title="หน้าที่มีคนเข้าเยอะ"
          icon={Eye}
          rows={topPages}
          name={(row) => String(row.path || row.url || row.page || '/')}
          value={(row) => `${fmt(row.views ?? row.count ?? row.pageviews)} ครั้ง`}
        />
      </div>
    </div>
  );
...
function InsightPanel({
  ...
  return (
    <aside className="flex min-h-[22rem] flex-col rounded-2xl border bg-white p-4 shadow-sm">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-xs font-black uppercase tracking-wider text-violet-700">AI ANALYTICS</p>
        ...
        <button
          ...
          disabled={busy}
          onClick={onRefresh}
          className="rounded-xl border px-2.5 py-2 text-xs font-bold hover:bg-slate-50 disabled:opacity-60"
        >
      ...
      {insights.length ? (
        <div className="mt-3 space-y-2 overflow-hidden">
          {insights.slice(0, 5).map((item: Row) => (
            <article
              key={String(item.id || item.title || item.badge || item.text || 'insight')}
              className={`rounded-xl p-3 ${item === insights[0] ? 'border border-emerald-100 bg-emerald-50/80' : 'border bg-slate-50/70'}`}
            >
              <strong className="block text-xs font-black text-slate-950">
                {item.title || item.badge || 'Insight'}
              </strong>
        ...
        <div className="mt-3 flex flex-1 items-center rounded-xl bg-slate-50 p-4 text-sm text-slate-500">
          {busy ? 'กำลังให้ AI วิเคราะห์ข้อมูล โดยกราฟและตัวเลขด้านบนยังใช้งานได้ตามปกติ…' : insightMessage(status)}
        </div>
      )}
      ...
      <button
        ...
        onClick={onRefresh}
        disabled={busy}
        className="mt-auto inline-flex items-center justify-center rounded-xl bg-emerald-950 px-3 py-2.5 text-xs font-black text-white transition hover:bg-emerald-800 disabled:opacity-60"
      >
        ดูรายงาน AI เพิ่มเติม →
      </button>
    </aside>
  );
}
...
function CombinedTrendChart({ rows }: { rows: Row[] }) {
  ...
  useEffect(() => {
    ...
    chart.setOption({
      ...
      legend: { top: 0, data: ['ยอดขาย (บาท)', 'ผู้เข้าชม (คน)'] },
      grid: { left: 58, right: 58, top: 52, bottom: 55 },
      xAxis: {
        type: 'category',
        data: rows.map((row) => String(row.date || row.day || '')),
      ...
      yAxis: [
        ...
        { type: 'value', name: 'ผู้เข้าชม (คน)', position: 'right', splitLine: { show: false } },
      ],
      series: [
        {
          name: 'ยอดขาย (บาท)',
  ...
  return (
    <section className="rounded-2xl border bg-white p-5 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-xs font-black uppercase tracking-wider text-emerald-700">PERFORMANCE OVERVIEW</p>
          <h2 className="mt-1 text-lg font-black">ยอดขาย & ผู้เข้าชม</h2>
          <p className="mt-1 text-xs text-slate-500">เปรียบเทียบแนวโน้มสองตัวชี้วัดในช่วงเวลาที่เลือก</p>
        </div>
        <span className="rounded-xl bg-emerald-50 px-3 py-2 text-xs font-bold text-emerald-800">
          รวมกราฟแล้ว
        </span>
      </div>
      <div ref={ref} className="mt-3 h-[22rem] w-full" />
    </section>
  );
}
...
function Sparkline({ values, color }: { values: unknown[]; color: string }) {
  ...
  const points = numeric
    .map((value, index) => `${(index / Math.max(numeric.length - 1, 1)) * 100},${28 - (value / max) * 24}`)
    .join(' ');
  return (
    <svg
      className="pointer-events-none absolute bottom-3 right-3 h-9 w-20 opacity-85"
      viewBox="0 0 100 32"
      aria-hidden="true"
    >
      <polyline
        points={points || '0,28 100,28'}
        fill="none"
        stroke={color}
        strokeLinecap="round"
...
function Ranking({
  ...
  icon: Icon,
  rows,
  name = (row) => String(row.name || row.product_name || row.product_id || '—'),
  value,
}: {
  title: string;
  icon: typeof Eye;
    ...
    <section className="rounded-2xl border bg-white p-5 shadow-sm">
      ...
      <ol className="mt-4 space-y-2">
        {rows.slice(0, 7).map((row, index) => (
          <li
            key={String(row.product_id || row.path || index)}
            className="grid grid-cols-[28px_1fr_auto] items-center gap-2 rounded-xl bg-slate-50 p-3"
          >
            <span className="grid size-7 place-items-center rounded-full bg-white text-xs font-black">
              {index + 1}
            </span>
            <strong className="line-clamp-2 text-xs">{name(row)}</strong>
            <span className="text-right text-[10px] font-bold text-slate-500">{value(row)}</span>
          </li>
        ))}
        {!rows.length && <li className="py-10 text-center text-sm text-slate-400">ยังไม่มีข้อมูล</li>}
      </ol>
    </section>
  );
}
