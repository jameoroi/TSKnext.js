...
const expectedApis = [
  ...
  '/api/v1/[...path]',
  '/api/v1',
  // Intentional additions since the 15-route baseline: admin backup control
  // plane and the tenant-safe tax-invoice endpoint.
...
function routeFromPage(file) {
  // Pages only exist under src/app. A repo-wide walk once counted a stale
  // duplicate page.tsx at the repository root as a ghost route, so scope here.
  const appRoot = path.join(root, 'src', 'app') + path.sep;
...
const pageRoutes = new Set(
  files
    .filter((f) => f.endsWith(`${path.sep}page.tsx`) || f.endsWith('/page.tsx'))
    .map(routeFromPage)
    .filter(Boolean),
);
const apiRoutes = new Set(
  files
    .filter(
      (f) =>
        f.includes(`${path.sep}src${path.sep}app${path.sep}api${path.sep}`) &&
...
pass('189 commerce actions', actionSet.size === 189, `${actionSet.size}`);
for (const action of businessCriticalActions) pass(`Business action ${action}`, actionSet.has(action));

const requiredFiles = [
  'src/components/site/header.tsx',
  'src/components/site/storefront-chrome.tsx',
  'src/components/site/trust-strip.tsx',
  'src/components/commerce/cart-view.tsx',
  'src/components/commerce/checkout-form.tsx',
  'src/components/commerce/product-detail-actions.tsx',
  'src/components/kits/equipment-kit-builder.tsx',
  'src/components/admin/kit-manager.tsx',
  'src/components/admin/operations-manager.tsx',
  ...
  'src/components/admin/backend-status.tsx',
  'src/server/equipment-kits.ts',
  'src/shared/kit-quote.mjs',
...
const activeCode = files.filter(
  (f) => f.includes(`${path.sep}src${path.sep}`) && /\.(?:ts|tsx|js|mjs)$/.test(f),
);
...
pass(
  ...
  pkg.dependencies?.react || 'missing',
);
