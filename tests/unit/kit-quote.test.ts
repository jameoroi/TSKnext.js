...
describe('signed equipment-kit quote', () => {
  it('accepts a valid same-tenant cart claim', () => {
    ...
    const token = signKitQuote({
      ...
      set_name: 'ชุดช่างไฟ',
      discount_amount: 250,
      eligible_subtotal: 2500,
      items: [
        { id: 'p1', variant_id: '', qty: 1 },
        { id: 'p2', variant_id: 'v2', qty: 2 },
      ],
    });
    const result = verifyKitQuote(token, {
      tenantId: 'tsk',
      items: [
        { id: 'p1', qty: 1 },
        { id: 'p2', variant_id: 'v2', qty: 2 },
  ...
  it('rejects changed carts and cross-tenant reuse', () => {
    ...
    expect(verifyKitQuote(token, { tenantId: 'tsk', items: [{ id: 'p1', qty: 1 }] })).toMatchObject({
      ok: false,
      error: 'kit_quote_cart_changed',
    });
    expect(verifyKitQuote(token, { tenantId: 'other', items: [{ id: 'p1', qty: 2 }] })).toMatchObject({
      ok: false,
      error: 'kit_quote_tenant',
    });
  });
});
