...
export default async function Page() {
  ...
  return <>
    <AdminPageHeader title="แชตลูกค้า" description="Telegram live chat + Facebook Messenger inbox · reply · polling · audited staff communication"/>
    <div className="grid gap-6"><ChatManager
      initialConversations={data.conversations || []}
      csrf={String(session.csrf || '')}
      owner={session.admin_role === 'super_admin'}
    />
    <FacebookInbox initial={facebook.conversations || []} csrf={String(session.csrf || '')}/></div>
  </>;
}
