...
export default async function Page() {
  const [data, session] = await Promise.all([
    safeLegacy<any>('admin.coupons.list', {}, { coupons: [] }),
    getLegacySession(),
  ]);
  return (
    <>
      <AdminPageHeader title="คูปอง" description="Discount rules · shipping promotions · limits · schedule" />
      <ResourceManager
        title="คูปอง"
        initialRows={data.coupons || []}
        csrf={String(session.csrf || '')}
        idKey="code"
        createAction="admin.coupons.save"
        updateAction="admin.coupons.save"
