...
export async function GET() {
  const services = {
    ...
    commerceStorage: process.env.COMMERCE_STORAGE || (databaseConfigured() ? 'postgres' : (process.env.SUPABASE_URL ? 'supabase' : 'not-configured')),
    ...
    objectStorage: Boolean((process.env.MEDIA_S3_ENDPOINT || process.env.S3_ENDPOINT) && (process.env.MEDIA_BUCKET || process.env.S3_BUCKET)),
  ...
  return NextResponse.json({
    ...
    next: '16.3.4',
    services,
    build: process.env.VERCEL_GIT_COMMIT_SHA?.slice(0, 12) || process.env.CF_PAGES_COMMIT_SHA?.slice(0, 12) || 'local',
