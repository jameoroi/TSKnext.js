...
function tokens(value: string) {
  return text(value).split(/[\s,./|;:()\[\]{}\-_]+/).map((item) => item.trim()).filter((item) => item.length >= 2);
}

function scoreCandidate(product: Candidate, request: z.infer<typeof schema>) {
  const haystack = text([product.name, product.brand, product.category, product.sku, JSON.stringify(product.specs || {})].join(' '));
  let score = 0;
  ...
  if (request.level === 'expert' && /pro|brushless|heavy|industrial|professional|ไร้แปรง|งานหนัก/.test(haystack)) score += 3;
  return score;
}
...
function ruleBased(request: z.infer<typeof schema>, candidates: Candidate[]) {
  ...
  const picked: Array<{ id: string; qty: number; reason: string; priority: 'essential' | 'recommended' | 'optional' }> = [];
  let total = 0;
  ...
  return {
    ...
    summary: request.budget > 0 ? `คัดจากสินค้าที่มีสต็อกจริงและพยายามให้อยู่ในงบ ${request.budget.toLocaleString('th-TH')} บาท` : 'คัดจากสินค้าที่มีสต็อกจริง โดยยังไม่ได้จำกัดงบประมาณ',
...
export async function POST(request: Request) {
  ...
  const candidates: Candidate[] = catalog.products.map((product) => ({
  ...
  if (!candidates.length) return Response.json({ ...fallback, warnings: ['ยังโหลดแคตตาล็อกสินค้าไม่ได้ กรุณาตรวจการเชื่อมต่อ API / DATABASE'] }, { status: 503 });

  const provider = aiSdkOpenAI();
  if (!provider) return Response.json(fallback);

  const compact = candidates.slice(0, 120).map(({ specs, ...row }) => ({ ...row, specs: Object.entries(specs).slice(0, 8) }));
  try {
    ...
    const items = Array.isArray(raw.items) ? raw.items.flatMap((entry: any) => {
      ...
      return [{
        ...
        priority: ['essential', 'recommended', 'optional'].includes(String(entry?.priority)) ? entry.priority : 'recommended',
      }];
    }) : [];
    ...
    console.warn('[kits.ai] falling back to deterministic planner', error instanceof Error ? error.message : error);
    return Response.json({ ...fallback, warnings: [...fallback.warnings, 'AI ไม่พร้อมใช้งานชั่วคราว ระบบจึงจัดชุดด้วยกฎจากแคตตาล็อกแทน'] });
  }
}
