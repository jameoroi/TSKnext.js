...
const fail = (error: string, status: number, detail?: unknown) =>
  json({ ok: false, error, ...(detail === undefined ? {} : { detail }) }, status);
const limitOf = (raw: string | null, fallback: number, max: number) =>
  Math.min(max, Math.max(1, Number(raw) || fallback));
// LIKE wildcards in the caller's pocket: % and _ inside the query are pattern
// operators, so `100%` matches `1000` and `_` matches anything. Escape them
// (with an explicit ESCAPE clause at each use site) so search means literal.
const likePattern = (query: string) => `%${query.replace(/[\\%_]/g, (char) => `\\${char}`)}%`;
...
export async function GET(request: NextRequest, ctx: Ctx) {
  ...
  if (!process.env.DATABASE_URL && resource !== 'health' && resource !== undefined) {
    return fail(
      ...
      503,
      'Set DATABASE_URL. The compatibility API at /api remains available.',
    );
  }
  ...
  try {
      ...
      return json(
        {
          ...
          product: {
            ...
            barcode: row.barcode,
            detail_images: row.detailImages,
            specs: row.specs,
            stock: row.stock,
    ...
    if (resource === 'products') {
      ...
      return json(
        {
          ok: true,
          products: page.map(shapeProduct),
      ...
      return json(
        {
          ...
          stock: {
            ...
            on_hand: row.stock,
            reserved: row.reserved,
            sellable: row.status === 'active' && available > 0,
          },
        },
        200,
        cacheHeaders(10),
    ...
    if (resource === 'search') {
      ...
      const { rows, matched } = await withDb(async (db) => {
        const exact = await db
          .select()
          .from(products)
          .where(and(live, sql`${products.name} ILIKE ${likePattern(query)} ESCAPE '\\'`))
          .limit(limit);
    ...
    if (resource === 'kits' && key) {
      ...
      const items = await withDb((db) =>
        db
          ...
          .where(and(eq(equipmentSetItems.tenantId, tenantId), eq(equipmentSetItems.setId, set.id)))
      ...
      return json(
        {
          ...
          kit: {
            ...
            discount_value: Number(set.discountValue || 0),
    ...
    if (resource === 'kits') {
      ...
      const rows = await withDb((db) =>
        db
          ...
          .orderBy(asc(equipmentSets.name))
          .limit(limit),
      );
      return json(
        {
    ...
    if (resource === 'categories') {
      const rows = await withDb((db) =>
        db
          .select()
          .from(categories)
          .where(and(eq(categories.tenantId, tenantId), eq(categories.active, true)))
    ...
    if (resource === 'brands') {
      const rows = await withDb((db) =>
        db
          .select()
          .from(brands)
          .where(and(eq(brands.tenantId, tenantId), eq(brands.active, true)))
    ...
    // Never reflect driver internals: Drizzle/Postgres messages name tables,
    // constraints and connection state — error-based reconnaissance. The full
    // error stays in the server log above; clients get a stable code.
