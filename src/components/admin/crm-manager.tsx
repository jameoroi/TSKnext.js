...
export function CrmManager({
  ...
  const filteredContacts = useMemo(() => {
    ...
    return contacts.filter((row) =>
      [row.name, row.email, row.phone, row.subject, row.message].some((value) =>
        String(value || '')
          .toLowerCase()
          .includes(needle),
      ),
  ...
  const activeSubscribers = useMemo(
    () => subscribers.filter((row) => row.status !== 'unsubscribed' && row.status !== 'inactive'),
    [subscribers],
  );
  ...
  async function sendNewsletter(event: FormEvent) {
    event.preventDefault();
    if (!owner) {
      setError('การส่ง Newsletter ใช้ได้เฉพาะ Super Admin');
      return;
    }
    if (
      !window.confirm(
        `ยืนยันส่ง Newsletter ถึงสมาชิกที่ Active ประมาณ ${activeSubscribers.length.toLocaleString('th-TH')} ราย?`,
      )
    )
      return;
    setBusy('newsletter');
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <Metric
          ...
          note="พร้อมรับ Newsletter"
        />
        <button
          type="button"
          onClick={() => void reload()}
          disabled={busy === 'reload'}
          className="flex items-center justify-center gap-2 rounded-2xl border bg-white p-4 text-sm font-black shadow-sm"
        >
      ...
      <section className="overflow-hidden rounded-2xl border bg-white shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b p-5">
          <div>
            <h2 className="font-black">Contact Inbox</h2>
            <p className="mt-1 text-xs text-slate-500">ข้อความจากลูกค้า พร้อมปุ่มตอบกลับผ่าน Email</p>
          </div>
          <label className="relative min-w-[240px]">
            ...
            <input
              ...
              placeholder="ค้นหาชื่อ อีเมล เบอร์ หรือหัวข้อ"
            />
          </label>
        </div>
          ...
          <table className="w-full min-w-[980px] text-sm">
            ...
            <tbody>
              {filteredContacts.map((row, index) => (
                <tr key={String(row.id || `${row.email}-${index}`)} className="border-t align-top">
                  <td className="px-4 py-3">
                    <strong>{row.name || '—'}</strong>
                    <p className="mt-1 text-xs text-slate-500">{row.email || '—'}</p>
                    <p className="text-xs text-slate-400">{row.phone || ''}</p>
                  </td>
                  <td className="px-4 py-3 font-semibold">{row.subject || 'ติดต่อร้านค้า'}</td>
                  <td className="max-w-[420px] px-4 py-3 whitespace-pre-wrap text-xs leading-5 text-slate-600">
                    {row.message || '—'}
                  </td>
                  ...
                  <td className="px-4 py-3 text-right">
                    <button
                      ...
                      disabled={!row.email}
                      onClick={() => replyTo(row)}
                      className="rounded-lg border px-2.5 py-1.5 text-xs font-black disabled:opacity-40"
                    >
                      ตอบอีเมล
                    </button>
                  </td>
      ...
      <div className="grid gap-6 xl:grid-cols-[.9fr_1.1fr]">
        <form onSubmit={sendDirect} className="rounded-2xl border bg-white p-5 shadow-sm">
          <div className="flex items-center gap-2">
            <Send size={17} className="text-emerald-700" />
            <div>
              <h2 className="font-black">ส่ง Email รายบุคคล</h2>
              <p className="text-xs text-slate-500">ใช้ SMTP ที่ตั้งไว้ใน Settings และเก็บ email log ฝั่ง API</p>
            </div>
          </div>
          <div className="mt-4 space-y-3">
            ...
            <button
              ...
              className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-emerald-950 px-4 py-3 text-sm font-black text-white"
            >
              <Send size={15} />
        ...
        <section className="rounded-2xl border bg-white p-5 shadow-sm">
          <div className="flex items-center gap-2">
            <Megaphone size={17} className="text-violet-700" />
            <div>
              <h2 className="font-black">Newsletter Broadcast</h2>
              <p className="text-xs text-slate-500">ส่งแบบ BCC เป็น batch ไปยังสมาชิก Active</p>
            </div>
          </div>
          <form onSubmit={sendNewsletter} className="mt-4 space-y-3">
            <Field label="หัวข้อ Newsletter">
              <input
                required
            ...
            <button
              disabled={busy === 'newsletter' || !owner}
              className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-violet-700 px-4 py-3 text-sm font-black text-white disabled:opacity-40"
            >
              <Megaphone size={15} />
              {busy === 'newsletter'
                ? 'กำลังส่ง…'
          ...
          <div className="mt-5 max-h-[280px] overflow-auto rounded-xl border">
            <table className="w-full min-w-[520px] text-xs">
              ...
              <tbody>
                {subscribers.map((row, index) => (
                  <tr key={String(row.id || row.email || index)} className="border-t">
                    <td className="px-3 py-2 font-semibold">{row.email || '—'}</td>
                    <td className="px-3 py-2">{row.status || 'active'}</td>
                    <td className="px-3 py-2 text-slate-500">{when(row.created_at)}</td>
                  </tr>
                ))}
                {!subscribers.length && (
                  <tr>
                    <td colSpan={3} className="p-8 text-center text-slate-400">
                      ยังไม่มีสมาชิก Newsletter
                    </td>
                  </tr>
                )}
              </tbody>
      ...
      <style
        jsx
      >{`.input{margin-top:.25rem;height:2.75rem;width:100%;border-radius:.75rem;border:1px solid rgb(226 232 240);padding:0 .75rem;font-size:.875rem}.textarea{margin-top:.25rem;width:100%;border-radius:.75rem;border:1px solid rgb(226 232 240);padding:.75rem;font-size:.875rem}`}</style>
    </div>
  );
}
