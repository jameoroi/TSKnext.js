...
function rowsFor(products: Row[], warehouseId: string): InventoryRow[] {
  ...
  for (const product of products || []) {
    ...
    for (const variant of variants) {
      const level = variant?.inventory?.[warehouseId] || {
        on_hand: 0,
        reserved: 0,
        reorder_point: product.low_stock_threshold ?? 5,
      };
...
export function InventoryManager({
  ...
  const [warehouseDraft, setWarehouseDraft] = useState<WarehouseRow>({
    id: '',
    code: '',
    name: '',
    address: '',
  ...
  const allRows = useMemo(
    () => rowsFor(data.products || [], selectedWarehouse),
    [data.products, selectedWarehouse],
  );
  const visibleRows = useMemo(() => {
    ...
    return allRows
      .filter(
        (row) =>
          !needle ||
          `${row.productName} ${row.variantLabel} ${row.sku} ${row.barcode}`.toLowerCase().includes(needle),
      )
  ...
  async function adjust(row: InventoryRow) {
    ...
    if (row.onHand === null) {
      setError('รุ่นนี้เป็นสต็อกไม่จำกัด');
      return;
    }
    ...
    try {
      ...
      setData((current: any) => ({
        ...current,
        products: (current.products || []).map((product: Row) =>
          String(product.id) === row.productId ? result.product : product,
        ),
      }));
      setDraft((current) => {
        const nextDraft = { ...current };
        delete nextDraft[row.key];
        return nextDraft;
      });
  ...
  function openTransfer(row: InventoryRow) {
    const target = warehouses.find(
      (warehouse) => warehouse.active !== false && warehouse.id !== row.warehouseId,
    );
    setTransferRow(row);
  ...
  async function transfer() {
    ...
    try {
      ...
      setData((current: any) => ({
        ...current,
        products: (current.products || []).map((product: Row) =>
          String(product.id) === transferRow.productId ? result.product : product,
        ),
      }));
      setNotice(
        `โอน ${transferRow.variantLabel} จำนวน ${qty.toLocaleString('th-TH')} ชิ้นแล้ว · Transfer ${result.transfer_id || ''}`,
      );
      setTransferRow(null);
    } catch (err) {
      setError(friendly(err));
  ...
  async function openLogs(row?: InventoryRow) {
    ...
    try {
      ...
      setError(`โหลดประวัติไม่สำเร็จ: ${friendly(err)}`);
    } finally {
      setBusy('');
    }
  }
  ...
  function newWarehouse() {
    setWarehouseDraft({
      ...
      name: '',
      address: '',
      priority: Math.max(2, warehouses.length + 1),
      active: true,
    });
  ...
  async function saveWarehouse() {
    if (!warehouseDraft.code.trim() || !warehouseDraft.name.trim()) {
      setError('กรุณากรอกรหัสคลังและชื่อคลัง');
      return;
    }
    setBusy('warehouse-save');
    clearMessage();
    try {
      await legacyRequest(
        'admin.warehouses.save',
        {
          ...warehouseDraft,
  ...
  async function migrateLegacyInventory() {
    ...
    const confirmed = window.confirm(
      'Normalize โครงสร้างคลังของสินค้าทั้งหมด? การทำงานนี้เป็น one-way maintenance สำหรับข้อมูลเก่าและจะถูกบันทึกใน Audit Log',
    );
    if (!confirmed) return;
    setBusy('inventory-migrate');
    clearMessage();
    try {
  ...
  const otherWarehouses = warehouses.filter(
    (warehouse) => warehouse.id !== selectedWarehouse && warehouse.active !== false,
  );
  ...
  return (
    <div className="space-y-5">
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <Metric
          label="พร้อมขาย"
          value={metrics.available.toLocaleString('th-TH')}
          note={`คลัง ${currentWarehouse?.name || selectedWarehouse}`}
        />
        <Metric label="ถูกจอง" value={metrics.reserved.toLocaleString('th-TH')} note="ยังไม่หักออกจาก on-hand" />
        <Metric
          label="ใกล้หมด"
          value={metrics.low.toLocaleString('th-TH')}
          note="ต่ำกว่า reorder point"
          warning={metrics.low > 0}
        />
        <Metric
          label="หมดสต็อก"
          value={metrics.out.toLocaleString('th-TH')}
          note="available = 0"
          danger={metrics.out > 0}
        />
      </div>
      ...
      <section className="rounded-2xl border bg-white p-4 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="font-black">Multi-Warehouse Inventory</h2>
              <p className="text-xs text-slate-500">นับสต็อกระดับ Variant × Warehouse พร้อมยอดจองและ available</p>
            </div>
          </div>
          <div className="flex flex-wrap gap-2">
            ...
            {owner && (
              <button
                type="button"
                onClick={() => void migrateLegacyInventory()}
                disabled={busy === 'inventory-migrate'}
                className="inline-flex items-center gap-2 rounded-xl border border-amber-300 bg-amber-50 px-3 py-2 text-xs font-black text-amber-900"
                title="One-way maintenance สำหรับ normalize inventory schema ของข้อมูลเก่า"
              >
                <RefreshCcw size={14} className={busy === 'inventory-migrate' ? 'animate-spin' : ''} />
                Normalize ข้อมูลเก่า
              </button>
            )}
            <button
              type="button"
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {warehouses.map((warehouse) => (
            <div
              key={warehouse.id}
              className={`flex items-center gap-1 rounded-xl border p-1 ${selectedWarehouse === warehouse.id ? 'border-emerald-700 bg-emerald-50' : 'bg-white'}`}
            >
              <button
                type="button"
                onClick={() => {
        ...
        <form
          ...
          className="mt-4 flex flex-wrap items-center gap-2 rounded-2xl bg-slate-50 p-3"
        >
          ...
          <label className="inline-flex items-center gap-2 rounded-xl border bg-white px-3 py-2 text-xs font-bold">
            <input type="checkbox" checked={lowOnly} onChange={(event) => setLowOnly(event.target.checked)} />
            เฉพาะใกล้หมด
          </label>
          <button
            className="h-10 rounded-xl border bg-white px-4 text-xs font-black"
            disabled={busy === 'reload'}
          >
            ค้นหา
          </button>
            ...
            <tbody>
              {visibleRows.map((row) => {
                ...
                return (
                  <tr
                    key={row.key}
                    className={`border-t align-middle ${out ? 'bg-rose-50/40' : low ? 'bg-amber-50/40' : ''}`}
                  >
                    <td className="px-4 py-3">
                      <p className="font-black text-slate-900">{row.productName}</p>
                      <p className="mt-1 text-xs text-slate-500">
                        {row.variantLabel} · <span className="uppercase">{row.productState}</span>
                      </p>
                    </td>
                    <td className="px-4 py-3">
                      <code className="text-xs">{row.sku || '—'}</code>
                      {row.barcode && <p className="mt-1 text-[10px] text-slate-400">{row.barcode}</p>}
                    </td>
                    <td className="px-4 py-3 text-right font-black">
                      {row.onHand === null ? '∞' : row.onHand.toLocaleString('th-TH')}
                    </td>
                    <td className="px-4 py-3 text-right text-amber-700">
                      {row.reserved.toLocaleString('th-TH')}
                    </td>
                    <td
                      className={`px-4 py-3 text-right font-black ${out ? 'text-rose-700' : low ? 'text-amber-700' : 'text-emerald-700'}`}
                    >
                      {row.available === null ? '∞' : row.available.toLocaleString('th-TH')}
                    </td>
                    <td className="px-4 py-3 text-right text-slate-500">
                      {row.reorderPoint.toLocaleString('th-TH')}
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          min={row.reserved}
                          step="1"
                          disabled={row.onHand === null || busy === row.key}
                          value={draft[row.key] ?? (row.onHand === null ? '' : String(row.onHand))}
                          onChange={(event) =>
                            setDraft((current) => ({ ...current, [row.key]: event.target.value }))
                          }
                          className="h-9 w-24 rounded-lg border bg-white px-2 text-right text-sm disabled:bg-slate-100"
                        />
                        <button
                          type="button"
                          disabled={
                            row.onHand === null ||
                            busy === row.key ||
                            (draft[row.key] ?? String(row.onHand ?? '')) === String(row.onHand ?? '')
                          }
                          onClick={() => void adjust(row)}
                          className="inline-flex h-9 items-center gap-1 rounded-lg bg-emerald-950 px-2.5 text-xs font-black text-white disabled:opacity-40"
                        >
                          <Save size={13} />
                          บันทึก
                        </button>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-1">
                        <button
                          type="button"
                          disabled={!otherWarehouses.length || row.available === null || row.available < 1}
                          onClick={() => openTransfer(row)}
                          className="inline-flex items-center gap-1 rounded-lg border px-2.5 py-2 text-xs font-bold disabled:opacity-40"
                        >
        ...
        <p className="mt-3 text-xs leading-5 text-slate-500">
          ระบบไม่อนุญาตให้ปรับ On hand ต่ำกว่ายอด Reserved เพื่อป้องกันออเดอร์ที่จองสต็อกไว้แล้วเสียหาย และการโอนคลังใช้
          Available เป็นเพดาน
        </p>
      </section>
      ...
      <section className="rounded-2xl border bg-white p-4 shadow-sm">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="font-black">คลังสินค้า</h2>
              <p className="text-xs text-slate-500">เปิด/ปิดคลัง กำหนดลำดับ และที่อยู่</p>
            </div>
          </div>
          <span className="text-xs font-bold text-slate-500">{warehouses.length} คลัง</span>
        </div>
        <div className="mt-3 grid gap-2 md:grid-cols-2 xl:grid-cols-3">
          {warehouses.map((warehouse) => (
            <article key={warehouse.id} className="rounded-2xl border p-4">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-sm font-bold">{warehouse.name}</p>
                  <p className="mt-2 text-xs leading-5 text-slate-500">
                    {warehouse.address || 'ยังไม่ได้ระบุที่อยู่'}
                    <br />
                ...
                <div className="flex gap-1">
                  ...
                  {!warehouse.is_default && (
                    <button
                      type="button"
                      disabled={busy === `warehouse:${warehouse.id}`}
      ...
      {transferRow && (
        <Modal title="โอนสต็อกระหว่างคลัง" onClose={() => setTransferRow(null)}>
          <div className="space-y-4">
            <div className="rounded-xl bg-slate-50 p-3 text-sm">
              <strong>{transferRow.productName}</strong>
              <p className="mt-1 text-xs text-slate-500">
                {transferRow.variantLabel} · {transferRow.sku || 'ไม่มี SKU'} · พร้อมโอนสูงสุด{' '}
                {transferRow.available?.toLocaleString('th-TH') || 0}
              </p>
            </div>
            <div className="grid gap-3 sm:grid-cols-2">
              <label className="text-xs font-bold text-slate-600">
                ...
                <input
                  readOnly
                  value={currentWarehouse?.name || selectedWarehouse}
                  className="mt-1 h-10 w-full rounded-xl border bg-slate-100 px-3 font-normal"
                />
              </label>
              <label className="text-xs font-bold text-slate-600">
                ไปคลัง
              ...
              <label className="text-xs font-bold text-slate-600">
                ...
                <input
                  type="number"
                  min="1"
                  max={transferRow.available ?? undefined}
                  value={transferQty}
            ...
            <button
              ...
              className="w-full rounded-xl bg-emerald-950 px-4 py-3 text-sm font-black text-white"
            >
              {busy === 'transfer' ? 'กำลังโอน…' : 'ยืนยันการโอน'}
            </button>
          </div>
        </Modal>
      )}
      ...
      {warehouseOpen && (
        <Modal
          title={warehouseDraft.id ? 'แก้ไขคลังสินค้า' : 'เพิ่มคลังสินค้า'}
        ...
        >
          <div className="grid gap-3 sm:grid-cols-2">
            <label className="text-xs font-bold text-slate-600">
              ...
              <input
                ...
                disabled={warehouseDraft.is_default}
                maxLength={40}
                onChange={(event) =>
                  setWarehouseDraft((current) => ({ ...current, code: event.target.value.toUpperCase() }))
                }
                className="mt-1 h-10 w-full rounded-xl border px-3 font-normal uppercase disabled:bg-slate-100"
                placeholder="BKK01"
              />
            </label>
            <label className="text-xs font-bold text-slate-600">
              ...
              <input
                ...
                maxLength={160}
                onChange={(event) =>
                  setWarehouseDraft((current) => ({ ...current, name: event.target.value }))
                }
                className="mt-1 h-10 w-full rounded-xl border px-3 font-normal"
              />
            </label>
            <label className="text-xs font-bold text-slate-600 sm:col-span-2">
              ...
              <textarea
                ...
                maxLength={500}
                onChange={(event) =>
                  setWarehouseDraft((current) => ({ ...current, address: event.target.value }))
                }
            ...
            <label className="text-xs font-bold text-slate-600">
              ...
              <input
                ...
                min="1"
                max="999"
                value={warehouseDraft.priority || 1}
                onChange={(event) =>
                  setWarehouseDraft((current) => ({ ...current, priority: Number(event.target.value) }))
                }
                className="mt-1 h-10 w-full rounded-xl border px-3 font-normal"
              />
            </label>
            <label className="flex items-end">
              <span className="flex h-10 w-full items-center gap-2 rounded-xl border px-3 text-xs font-bold">
                <input
                  type="checkbox"
                  checked={warehouseDraft.active !== false}
                  onChange={(event) =>
                    setWarehouseDraft((current) => ({ ...current, active: event.target.checked }))
                  }
                />
                เปิดใช้งานคลัง
              </span>
          ...
          <button
            type="button"
            disabled={busy === 'warehouse-save'}
            onClick={() => void saveWarehouse()}
            className="mt-4 w-full rounded-xl bg-emerald-950 px-4 py-3 text-sm font-black text-white"
          >
            {busy === 'warehouse-save' ? 'กำลังบันทึก…' : 'บันทึกคลัง'}
          </button>
        </Modal>
      )}

      {logsOpen && (
        <Modal title="Inventory Ledger" onClose={() => setLogsOpen(false)} wide>
          <div className="max-h-[65vh] overflow-auto rounded-xl border">
            <table className="w-full min-w-[900px] text-xs">
              <thead className="sticky top-0 bg-slate-50 text-left text-slate-500">
                <tr>
                  <th className="px-3 py-2">เวลา</th>
                  <th className="px-3 py-2">SKU / Variant</th>
              ...
              <tbody>
                {logs.map((log) => (
                  <tr key={log.id} className="border-t">
                    <td className="whitespace-nowrap px-3 py-2">{dateTime(log.at)}</td>
                    <td className="px-3 py-2">
                      <code>{log.sku || '—'}</code>
                      <div className="text-[10px] text-slate-400">{log.variant_id || 'default'}</div>
                    </td>
                    <td className="px-3 py-2">
                      {warehouses.find((warehouse) => warehouse.id === log.warehouse_id)?.code ||
                        log.warehouse_id ||
                        '—'}
                    </td>
                    <td className="px-3 py-2 text-right">{log.before ?? '—'}</td>
                    <td
                      className={`px-3 py-2 text-right font-black ${Number(log.delta) < 0 ? 'text-rose-700' : 'text-emerald-700'}`}
                    >
                      {Number(log.delta) > 0 ? '+' : ''}
                      {log.delta ?? 0}
                    </td>
                    <td className="px-3 py-2 text-right">{log.after ?? '—'}</td>
                    <td className="px-3 py-2">{log.reason || '—'}</td>
                    <td className="px-3 py-2 text-[10px] text-slate-500">
                      {log.order_no || log.transfer_id || log.admin || '—'}
                    </td>
                  </tr>
                ))}
                {!logs.length && (
...
function Modal({
  ...
  return (
    ...
    >
      <section
        className={`max-h-[90vh] w-full overflow-auto rounded-3xl bg-white p-5 shadow-2xl ${wide ? 'max-w-6xl' : 'max-w-2xl'}`}
      >
