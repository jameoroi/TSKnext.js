...
export function KitManager({ products }: { products: Product[] }) {
  ...
  async function load() {
    ...
    try {
      ...
      showToast(`โหลดชุดอุปกรณ์ไม่สำเร็จ: ${error instanceof Error ? error.message : 'unknown_error'}`, {
        tone: 'bad',
      });
    } finally {
      setLoading(false);
  ...
  async function save() {
    ...
    setSaving(true);
    try {
      const response = await fetch('/api/kits/manage', {
        method: 'POST',
        credentials: 'include',
  ...
  async function remove(id: string) {
    ...
    try {
      const response = await fetch(`/api/kits/manage?id=${encodeURIComponent(id)}`, {
        method: 'DELETE',
        credentials: 'include',
      });
      const data = await response.json();
      ...
      showToast(`ลบไม่สำเร็จ: ${error instanceof Error ? error.message : 'unknown_error'}`, { tone: 'bad' });
    }
  }
  ...
  function patchLine(id: string, patch: Partial<SetLine>) {
    setDraft((current) => ({
      ...current,
      items: current.items.map((item) => (item.productId === id ? { ...item, ...patch } : item)),
    }));
  }

  return (
    <div className="grid gap-6 2xl:grid-cols-[minmax(0,1.2fr)_460px]">
      <div className="space-y-5">
        <section className="rounded-2xl border bg-white p-5 shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <p className="text-xs font-black uppercase tracking-[.14em] text-violet-700">CURATED BUNDLES</p>
              <h2 className="mt-1 text-xl font-black">{draft.id ? 'แก้ไขชุดอุปกรณ์' : 'สร้างชุดอุปกรณ์ใหม่'}</h2>
              <p className="mt-1 text-sm text-slate-500">
                สร้างชุดสำเร็จรูป กำหนดสินค้าที่จำเป็น/เสริม ส่วนลด และ SEO แล้วเปิดขายบนหน้าร้าน
              </p>
            </div>
            {draft.id ? (
              <button
          ...
          <div className="mt-5 grid gap-4 md:grid-cols-2">
            <Field label="ชื่อชุด">
              <input
                ...
                placeholder="เช่น ชุดช่างไฟเริ่มต้น"
              />
            </Field>
            <Field label="Slug">
              <input
                ...
                className="h-11 w-full rounded-xl border px-3"
                placeholder="electrician-starter-kit"
              />
            </Field>
            <Field label="สถานะ">
              <select
                value={draft.status}
                onChange={(event) =>
                  setDraft((current) => ({ ...current, status: event.target.value as Draft['status'] }))
                }
                className="h-11 w-full rounded-xl border px-3"
              >
            ...
            <Field label="ส่วนลด">
              <div className="grid grid-cols-[150px_1fr] gap-2">
                ...
                >
                  ...
                  <option value="fixed">ลดบาท</option>
                </select>
                <input
                  type="number"
                  min="0"
        ...
        <section className="rounded-2xl border bg-white p-5 shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <h2 className="font-black">สินค้าในชุด ({draft.items.length})</h2>
              <p className="text-xs text-slate-500">Required = ต้องมีในชุด, Optional = ลูกค้าถอดออกได้</p>
            </div>
                  ...
                  <button
                    ...
                    onClick={() =>
                      setDraft((current) => ({
                        ...
                        items: [
                          ...current.items,
                          { productId: String(product.id), quantity: 1, required: true, note: '' },
                        ],
                      }))
                    }
                    className="flex items-center gap-2 rounded-xl border bg-white p-2 text-left hover:border-emerald-500"
                  >
                    <div className="relative size-12 shrink-0 rounded-lg bg-slate-50">
                      <Image
                    ...
                    <span className="min-w-0">
                      <strong className="line-clamp-2 text-xs">{product.name}</strong>
            ...
            {draft.items.map((line, index) => {
              ...
              return (
                <article
                  ...
                  className="grid gap-3 rounded-2xl border p-3 md:grid-cols-[64px_minmax(0,1fr)_100px_130px_40px] md:items-center"
                >
                  <div className="relative size-16 rounded-xl bg-slate-50">
                    {product ? (
                      <Image
                        src={productImage(product)}
                        alt=""
                  ...
                  <div className="min-w-0">
                    <strong className="line-clamp-2 text-sm">{product?.name || line.productId}</strong>
                    {product &&
                    ...
                    (product as any).variants.length ? (
                      <select
                        ...
                        onChange={(event) =>
                          patchLine(line.productId, { variantId: event.target.value || null })
                        }
                        className="mt-1 h-8 w-full rounded-lg border px-2 text-xs"
                      >
                        <option value="">ราคาหลัก / ไม่ล็อกตัวเลือก</option>
                        {(product as any).variants
                          .filter(
                            (variant: any) =>
                              variant?.state !== 'hidden' && variant?.state !== 'discontinued',
                          )
                          .map((variant: any) => (
                            <option
                              key={String(variant.id || variant.sku || variant.label)}
                              value={String(variant.id || '')}
                            >
                              {String(variant.label || variant.name || variant.sku || variant.id)} · ฿
                              {Number(variant.price ?? product.price ?? 0).toLocaleString('th-TH')}
                            </option>
                          ))}
                      </select>
                    ) : null}
                    <input
                      value={line.note || ''}
                      onChange={(event) => patchLine(line.productId, { note: event.target.value })}
                      className="mt-1 h-8 w-full rounded-lg border px-2 text-xs"
                      placeholder="หมายเหตุ / เหตุผลที่อยู่ในชุด"
                    />
                  </div>
                  <input
                    type="number"
                    ...
                    value={line.quantity}
                    onChange={(event) =>
                      patchLine(line.productId, { quantity: Math.max(1, Number(event.target.value) || 1) })
                    }
                  ...
                  <label className="flex items-center gap-2 text-xs font-bold">
                    <input
                      ...
                      onChange={(event) => patchLine(line.productId, { required: event.target.checked })}
                    />
                  ...
                  <button
                    ...
                    onClick={() =>
                      setDraft((current) => ({
                        ...current,
                        items: current.items.filter((item) => item !== line),
                      }))
                    }
          ...
          >
            ...
            {saving ? 'กำลังบันทึก…' : 'บันทึกชุดอุปกรณ์'}
          </Button>
        </section>
      </div>
      ...
      <aside className="space-y-4">
        <section className="rounded-2xl border bg-white p-4 shadow-sm">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="font-black">ชุดที่สร้างแล้ว</h2>
              <p className="text-xs text-slate-500">{sets.length} ชุด</p>
            </div>
            <button
              type="button"
              disabled={loading}
          ...
          <div className="mt-3 space-y-2">
            {sets.map((row) => (
              <article key={row.id} className="rounded-2xl border p-3">
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      <strong className="truncate text-sm">{row.name}</strong>
                      <span
                        className={`rounded-full px-2 py-0.5 text-[10px] font-black ${row.status === 'active' ? 'bg-emerald-100 text-emerald-800' : row.status === 'hidden' ? 'bg-slate-200 text-slate-700' : 'bg-amber-100 text-amber-700'}`}
                      >
                        {row.status}
                      </span>
                    </div>
                    <p className="mt-1 text-xs text-slate-400">
                      /{row.slug} · {row.items?.length || 0} รายการ
                    </p>
                  </div>
                  <div className="flex">
                    <button
            ...
            {loading ? (
              ...
              <p className="py-8 text-center text-sm text-slate-400">ยังไม่มีชุดที่บันทึก</p>
            ) : null}
          </div>
        </section>
        <section className="rounded-2xl bg-violet-950 p-5 text-white">
          <PackagePlus className="size-8 text-violet-200" />
          <h3 className="mt-3 font-black">Bundle rules</h3>
          <p className="mt-2 text-sm leading-6 text-white/65">
            ชุดที่เปิดขายใช้สินค้าใน catalog เดิม ทำให้ราคาและสต็อกอ้างอิงจากสินค้าจริง ไม่สร้าง inventory แยกที่เสี่ยงยอดคลาดเคลื่อน
          </p>
        </section>
      </aside>
    </div>
