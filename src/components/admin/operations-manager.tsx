...
function errorCode(error: unknown) {
  return error instanceof LegacyApiError ? error.code : error instanceof Error ? error.message : 'unknown';
}
...
function preferredKey(tab: string) {
  return (
    (
      {
        orders: 'orders',
...
export function OperationsManager({
  ...
  const [active, setActive] = useState(
    visibleTabs.some((tab) => tab.key === initialTab) ? initialTab : 'orders',
  );
  ...
  async function load(tab = activeTab) {
    ...
    setError('');
    try {
      setRecords(await legacyRequest<any>(tab.load, tab.key === 'customers' ? { q: '' } : {}));
    } catch (e) {
      setError(`โหลดข้อมูลไม่สำเร็จ: ${errorCode(e)}`);
    } finally {
      setLoading(false);
    }
  }
  ...
  async function write(action: string, payload: Record<string, unknown>) {
    ...
    try {
      ...
      setError(`ดำเนินการไม่สำเร็จ: ${errorCode(e)}`);
    } finally {
      setLoading(false);
    }
  }
  ...
  async function submitRaw(event: React.FormEvent) {
    ...
    try {
      ...
      setError(e instanceof SyntaxError ? 'Payload JSON ไม่ถูกต้อง' : `ดำเนินการไม่สำเร็จ: ${errorCode(e)}`);
    }
  }
  ...
  return (
    <div className="space-y-6">
      <section className="overflow-hidden rounded-2xl border bg-white shadow-sm">
        <div className="overflow-x-auto border-b bg-slate-50 p-2">
          <div className="flex min-w-max gap-1">
            {visibleTabs.map((tab) => (
              <button
                ...
                className={`rounded-xl px-3 py-2 text-xs font-bold ${active === tab.key ? 'bg-emerald-950 text-white' : 'text-slate-600 hover:bg-white'}`}
              >
                {tab.label}
              </button>
            ))}
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-b p-4">
          <div>
            <h2 className="font-bold">{activeTab.label}</h2>
            <p className="text-xs text-slate-500">
              {rows.length.toLocaleString('th-TH')} รายการ · {activeTab.load}
            </p>
          </div>
          <button
            type="button"
        ...
        {notice && (
          <p className="m-4 rounded-xl bg-emerald-50 p-3 text-sm font-semibold text-emerald-800">{notice}</p>
        )}
        {error && (
          <p className="m-4 rounded-xl bg-rose-50 p-3 text-sm font-semibold text-rose-700" role="alert">
            {error}
          </p>
        )}
        {loading && !rows.length ? (
          <p className="p-10 text-center text-sm text-slate-500">กำลังโหลด…</p>
        ) : (
          <RecordTable rows={rows} tab={active} busy={loading} write={write} />
        )}
      </section>
      ...
      <form onSubmit={submitRaw} className="rounded-2xl border border-amber-200 bg-amber-50/40 p-5 shadow-sm">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h2 className="font-bold">Advanced API Console</h2>
            <p className="mt-1 text-xs leading-5 text-slate-600">
              ใช้กับงานหลังบ้านที่ต้องระบุ payload ตาม API contract โดยตรง ระบบจะเติม CSRF ของ session ปัจจุบันให้ ทุก
              action ที่เขียนข้อมูลจะถูก audit ตาม backend เดิม
            </p>
          </div>
        </div>
        <div className="mt-4 grid gap-4 lg:grid-cols-[320px_1fr]">
          <label>
            <span className="mb-1.5 block text-sm font-semibold">Action</span>
            <select
              value={writeAction}
          ...
          <label>
            <span className="mb-1.5 block text-sm font-semibold">Payload JSON</span>
            <textarea
              value={payloadText}
              onChange={(e) => setPayloadText(e.target.value)}
              spellCheck={false}
              rows={7}
              className="w-full rounded-xl border bg-slate-950 p-3 font-mono text-xs text-slate-100"
            />
          </label>
        </div>
        <button
          disabled={loading}
          className="mt-4 inline-flex items-center gap-2 rounded-xl bg-emerald-950 px-4 py-2.5 text-sm font-bold text-white disabled:opacity-50"
        >
          <Send size={16} />
          ส่งคำสั่ง
        </button>
...
function RecordTable({
  ...
  tab: string;
  busy: boolean;
  write: (action: string, payload: Record<string, unknown>) => Promise<void>;
}) {
  const columns = useMemo(() => {
    ...
    const discovered = [
      ...new Set(
        rows.slice(0, 30).flatMap((row) => (row && typeof row === 'object' ? Object.keys(row) : [])),
      ),
    ];
    return [...new Set([...preferred, ...discovered])]
      .filter((key) => !/password|secret|token|hash|proof/i.test(key))
      .slice(0, 9);
  }, [rows, tab]);
  return (
      ...
      <table className="w-full min-w-[980px] text-sm">
        <thead className="bg-slate-50 text-left text-[11px] uppercase tracking-wide text-slate-500">
          <tr>
            ...
            {['slips', 'returns', 'reviews', 'media', 'customers'].includes(tab) && (
        ...
        <tbody>
          {rows.map((row, index) => (
            <tr key={String(row?.id || row?.key || row?.order_no || index)} className="border-t align-top">
              {columns.map((key) => (
              ...
              {['slips', 'returns', 'reviews', 'media', 'customers'].includes(tab) && (
                <td className="px-4 py-3">
                  <RowActions row={row} tab={tab} disabled={busy} write={write} />
                </td>
              )}
            </tr>
          ))}
...
function Cell({ value }: { value: unknown }) {
  ...
  if (typeof value === 'object')
    return (
      <code className="block max-h-20 overflow-hidden whitespace-pre-wrap break-all text-[10px] text-slate-500">
  ...
  if (/^https?:\/\//.test(text))
    return (
      <a href={text} target="_blank" rel="noreferrer" className="text-emerald-800 underline">
        เปิด
      </a>
    );
...
function RowActions({
  ...
  if (tab === 'slips')
    return (
      <div className="flex justify-end gap-1">
        <button
          disabled={disabled}
          onClick={() => void write('admin.slip.verify', { id: row.id, approve: true })}
          className="rounded-lg bg-emerald-950 px-2.5 py-1.5 text-xs font-bold text-white"
        >
          อนุมัติ
        </button>
        <button
          disabled={disabled}
          onClick={() => void write('admin.slip.verify', { id: row.id, approve: false })}
  ...
  if (tab === 'returns')
    return (
      <select
        disabled={disabled}
        value={String(row.status || 'requested')}
        onChange={(e) =>
          void write('admin.returns.status', {
            id: row.id,
            status: e.target.value,
            admin_note: row.admin_note || '',
          })
        }
        className="float-right rounded-lg border px-2 py-1 text-xs"
      >
  ...
  if (tab === 'reviews')
    return (
      <select
        ...
        onChange={(e) => void write('admin.reviews.moderate', { id: row.id, status: e.target.value })}
        className="float-right rounded-lg border px-2 py-1 text-xs"
      >
  ...
  if (tab === 'media')
    return (
      <button
        disabled={disabled}
        onClick={() =>
          window.confirm(`ลบไฟล์ ${row.key || ''}?`) && void write('admin.media.delete', { key: row.key })
        }
        className="float-right rounded-lg border p-2 text-rose-700"
        aria-label="ลบไฟล์"
      >
        <Trash2 size={15} />
      </button>
    ...
    return (
      <>
        <div className="flex justify-end gap-1">
          ...
          >
            {detailBusy ? 'กำลังโหลด…' : 'ดูออเดอร์'}
          </button>
          <button
            disabled={disabled}
            onClick={() => {
              const note = window.prompt('บันทึกภายในสำหรับลูกค้ารายนี้', String(row.admin_note || ''));
              if (note !== null) void write('admin.customer.note', { id: row.id, note });
            }}
            className="rounded-lg border px-2.5 py-1.5 text-xs font-bold"
          >
            บันทึกโน้ต
        ...
        {customerOrders !== null && (
          <div
            ...
            role="dialog"
            aria-modal="true"
            aria-label="คำสั่งซื้อของลูกค้า"
          >
            <div className="max-h-[82vh] w-full max-w-4xl overflow-hidden rounded-2xl bg-white shadow-2xl">
              <div className="flex items-start justify-between gap-4 border-b p-5">
                <div>
                  <h3 className="font-black">คำสั่งซื้อของ {row.name || row.email || row.id}</h3>
                  <p className="mt-1 text-xs text-slate-500">
                    Customer ID: {row.id || '—'} · {customerOrders.length.toLocaleString('th-TH')} รายการ
                  </p>
                </div>
                <button
              ...
              {detailError && (
                <p className="m-4 rounded-xl bg-rose-50 p-3 text-sm text-rose-700">{detailError}</p>
              )}
                ...
                <table className="w-full min-w-[760px] text-sm">
                  ...
                  <tbody>
                    {customerOrders.map((order, index) => (
                      <tr key={String(order.id || order.order_no || index)} className="border-t">
                        <td className="px-4 py-3 font-bold">{order.order_no || order.id || '—'}</td>
                        <td className="px-4 py-3">{order.status || '—'}</td>
                        <td className="px-4 py-3">{order.payment_status || order.payment_method || '—'}</td>
                        <td className="px-4 py-3 text-right font-black">
                          ฿{Number(order.total || 0).toLocaleString('th-TH', { maximumFractionDigits: 2 })}
                        </td>
                        <td className="px-4 py-3 text-xs">
                          {order.tracking_number || order.fulfillment?.[0]?.tracking_number || '—'}
                        </td>
                    ...
                    {!customerOrders.length && (
                      <tr>
                        <td colSpan={6} className="p-10 text-center text-slate-400">
                          ลูกค้ารายนี้ยังไม่มีคำสั่งซื้อ
                        </td>
                      </tr>
                    )}
                  </tbody>
