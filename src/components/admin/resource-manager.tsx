...
export function ResourceManager({
  ...
  const open = (row?: Row) => {
    ...
    if (row) setForm({ ...row });
      ...
      for (const field of fields) {
        ...
        if (field.type === 'select' && defaults[field.key] === undefined && field.options?.length)
  ...
  async function save() {
    ...
    try {
      ...
      setRows((current) =>
        isNew
          ? [saved, ...current]
          : current.map((row) => (String(row[idKey]) === String(key) ? saved : row)),
      );
      setEditing(null);
  ...
  async function remove(row: Row) {
    if (
      !deleteAction ||
      !window.confirm(`ยืนยันการลบ ${String(row.name || row.code || row[idKey] || 'รายการนี้')} ?`)
    )
      return;
    setBusy(true);
    setNotice('');
    ...
    <section className="rounded-2xl border bg-white shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b p-4">
        <div>
          ...
          <p className="text-xs text-slate-500">{rows.length.toLocaleString('th-TH')} รายการ</p>
        </div>
        ...
        <table className="w-full min-w-[760px] text-sm">
          <thead className="bg-slate-50 text-left text-xs text-slate-500">
            <tr>
              ...
              {(updateAction || deleteAction || reorderAction) && (
          ...
          <tbody>
            {filtered.map((row, index) => (
              <tr key={String(row[idKey] ?? index)} className="border-t">
                {fields.slice(0, 6).map((field) => (
                  <td key={field.key} className="max-w-[280px] truncate px-4 py-3">
                    {renderValue(row[field.key], field)}
                  </td>
                ))}
                {(updateAction || deleteAction || reorderAction) && (
                        ...
                        <>
                          <button
                            ...
                            disabled={
                              busy ||
                              Boolean(q.trim()) ||
                              rows.findIndex((item) => String(item[idKey]) === String(row[idKey])) === 0
                            }
                            onClick={() => void move(row, -1)}
                            className="rounded-lg border p-2 disabled:opacity-30"
                            aria-label="เลื่อนขึ้น"
                            title={q.trim() ? 'ล้างคำค้นหาก่อนเรียงลำดับ' : 'เลื่อนขึ้น'}
                          >
                            <ArrowUp size={15} />
                          </button>
                          <button
                            ...
                            disabled={
                              busy ||
                              Boolean(q.trim()) ||
                              rows.findIndex((item) => String(item[idKey]) === String(row[idKey])) ===
                                rows.length - 1
                            }
                            onClick={() => void move(row, 1)}
                            className="rounded-lg border p-2 disabled:opacity-30"
                            aria-label="เลื่อนลง"
                            title={q.trim() ? 'ล้างคำค้นหาก่อนเรียงลำดับ' : 'เลื่อนลง'}
                          >
                            <ArrowDown size={15} />
                          </button>
                        </>
            ...
            {!filtered.length && (
              <tr>
                <td
                  ...
                  colSpan={Math.max(
                    1,
                    Math.min(fields.length, 6) + (updateAction || deleteAction || reorderAction ? 1 : 0),
                  )}
                >
                  ยังไม่มีข้อมูล
                </td>
      ...
      {editing && (
        ...
        >
          <div className="max-h-[88vh] w-full max-w-2xl overflow-y-auto rounded-3xl bg-white p-6 shadow-2xl">
            <div className="flex items-center justify-between">
              <h3 className="text-xl font-black">
                {editing[idKey] ? 'แก้ไข' : 'เพิ่ม'} {title}
              </h3>
              <button
                type="button"
            ...
            <div className="mt-5 grid gap-4 md:grid-cols-2">
              {fields.map((field) => (
                <FieldInput
                  ...
                  isEdit={Boolean(editing[idKey])}
                  csrf={csrf}
                  ownerId={String(form[idKey] ?? '')}
            ...
            <div className="mt-6 flex justify-end gap-2">
              ...
              <button
                ...
                disabled={busy}
                onClick={() => void save()}
                className="inline-flex items-center gap-2 rounded-xl bg-emerald-950 px-4 py-2 font-bold text-white disabled:opacity-50"
              >
                <Save size={16} />
    ...
    <label className={full ? 'md:col-span-2' : ''}>
      ...
      {field.type === 'checkbox' ? (
        ...
        <select
          ...
          onChange={(event) => onChange(event.target.value)}
          disabled={locked}
          className="h-11 w-full rounded-xl border bg-white px-3 disabled:bg-slate-100"
        >
        ...
        <div className="rounded-2xl border bg-slate-50 p-3">
          <div className="grid gap-3 sm:grid-cols-[130px_1fr]">
            <div className="grid aspect-square place-items-center overflow-hidden rounded-xl border bg-white">
              {String(value || '').trim() ? (
                <img src={String(value)} alt="" className="h-full w-full object-contain p-2" />
              ) : (
            ...
            <div className="grid content-start gap-2">
              <input
                ...
                className="w-full rounded-xl border bg-white p-2 text-xs disabled:opacity-50"
              />
              <input
                type="text"
                value={String(value ?? '')}
                ...
                className="h-10 w-full rounded-xl border bg-white px-3 text-xs read-only:bg-slate-100"
              />
              <div className="flex flex-wrap items-center gap-2">
                <span className="text-[10px] leading-4 text-slate-500">
                  {uploading ? 'กำลังย่อภาพและอัปโหลด…' : `ย่ออัตโนมัติไม่เกิน ${field.maxEdge || 1400}px ก่อนอัปโหลด`}
                </span>
                {String(value || '').trim() && !locked && (
                  <button
                    type="button"
                    onClick={() => onChange('')}
                    className="rounded-lg border bg-white px-2 py-1 text-[10px] font-bold text-rose-700"
                  >
                    ล้างรูป
                  </button>
                )}
        ...
        <input
          ...
          value={field.type === 'datetime-local' ? toLocalInput(value) : String(value ?? '')}
          onChange={(event) =>
          ...
          readOnly={locked}
          placeholder={field.placeholder}
          className="h-11 w-full rounded-xl border px-3 read-only:bg-slate-100 read-only:text-slate-500"
        />
      )}
...
function renderValue(value: unknown, field: FieldDef) {
  if (field.type === 'checkbox')
    return value !== false ? (
      <span className="font-bold text-emerald-700">เปิด</span>
    ) : (
  ...
  if (field.type === 'image')
    return (
      <img src={String(value)} alt="" className="h-10 w-14 rounded-lg border bg-white object-contain p-1" />
    );
