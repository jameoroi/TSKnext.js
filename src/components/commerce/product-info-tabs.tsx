...
export function ProductInfoTabs({
  ...
  return (
    <section className="mt-10 overflow-hidden rounded-3xl border bg-white shadow-sm">
              ...
              <div className="space-y-4 text-sm leading-7 text-slate-700">
                {blocks.map((block, index) =>
                  block.kind === 'list' ? (
                    ...
                    <p key={index} className="grid gap-1 rounded-xl bg-slate-50 p-3 sm:grid-cols-[180px_1fr]">
                    ...
                    {reviewImages.length > 0 && (
                      <div className="mt-3 flex flex-wrap gap-2">
                        {reviewImages.map((image, index) => (
                          ...
                          >
                            <img src={image} alt={`รูปรีวิว ${index + 1}`} className="size-full object-cover" />
                            <button
                              type="button"
                              onClick={() =>
