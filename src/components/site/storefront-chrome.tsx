...
export function StorefrontChrome() {
  ...
  useEffect(() => {
    ...
    return () => { window.removeEventListener('tsk:recent-products-change', load); window.removeEventListener('storage', load); };
  }, []);

  useEffect(() => { setRecentDismissed(false); }, [pathname]);

  const recentHidden = useMemo(() => ['/products/', '/cart', '/checkout', '/admin', '/partner', '/account', '/login'].some((prefix) => pathname.startsWith(prefix)), [pathname]);
  const showRecent = Boolean(recent && !recentDismissed && !recentHidden);

  return <>
    <nav className="fixed inset-x-0 bottom-0 z-40 grid h-16 grid-cols-5 border-t bg-white/95 shadow-[0_-8px_30px_rgba(15,23,42,.08)] backdrop-blur md:hidden" aria-label="เมนูลัด">
    ...
    {showTop && <button type="button" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })} className="back-to-top show fixed bottom-20 left-4 z-30 grid size-11 place-items-center rounded-full border bg-white text-emerald-950 shadow-lg md:bottom-5" aria-label="กลับขึ้นด้านบน"><ArrowUp size={18}/></button>}
  </>;
}

function Dock({ href, icon, label, badge = 0 }: { href: string; icon: React.ReactNode; label: string; badge?: number }) {
  return <Link href={href} className="relative grid place-items-center content-center gap-1 text-[10px] font-semibold text-slate-600 hover:text-emerald-800"><span className="relative">{icon}{badge > 0 && <i className="absolute -right-2 -top-2 min-w-4 rounded-full bg-rose-600 px-1 text-center text-[9px] not-italic leading-4 text-white">{badge > 99 ? '99+' : badge}</i>}</span>{label}</Link>;
}
