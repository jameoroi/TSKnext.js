  ...
  function addAllToCart() {
    for (const product of products) {
      const variants = Array.isArray((product as any).variants)
        ? (product as any).variants.filter(
      ...
      const stock = Number(
        variants[0]?.available ?? variants[0]?.stock ?? product.available ?? product.stock ?? 1,
      );
    ...
    <div className="mx-auto max-w-7xl px-4 py-10 lg:px-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          {mode === 'wishlist' && wishlist.signedIn && (
            <p className="mt-2 text-sm text-slate-500">ซิงก์กับบัญชีของคุณ ใช้รายการนี้ได้ทุกอุปกรณ์</p>
          )}
          {mode === 'compare' && (
            <p className="mt-2 text-sm text-slate-500">รายการเปรียบเทียบเก็บบนอุปกรณ์นี้ สูงสุด 12 รายการ</p>
          )}
        </div>
        <div className="flex gap-2">
          {products.length > 0 && (
            <Button variant="outline" onClick={addAllToCart}>
              ...
              เพิ่มสินค้าที่พร้อมขายลงตะกร้า
            </Button>
          )}
          {mode === 'compare' && ids.length > 0 && (
      ...
      {loading ? (
        ...
        <div className="mt-8 rounded-2xl border border-dashed bg-white p-10 text-center">
          <p className="text-slate-500">ยังไม่มีสินค้าในรายการ</p>
          <Link className={`${buttonVariants()} mt-4`} href="/products">
            เลือกดูสินค้า
          </Link>
        </div>
      ) : (
