...
import { onConsentChange, readConsent, type ConsentPreferences } from '@/features/privacy/consent';

const floodlightId = (value: unknown) => {
  const raw = String(value || '').trim().replace(/^DC-/i, '');
  return raw ? `DC-${raw}` : '';
};
...
export function MarketingRuntime() {
  ...
  useEffect(() => {
    ...
    if (!key) return;
    if (!posthog.__loaded) posthog.init(key, { api_host: process.env.NEXT_PUBLIC_POSTHOG_HOST || 'https://us.i.posthog.com', capture_pageview: false, autocapture: true, persistence: 'localStorage+cookie' });
  ...
  useEffect(() => {
    ...
    async function start() {
      const response = await fetch('/api?action=site.settings&compact=1', { credentials: 'include' }).catch(() => null);
      ...
      if (ga4 || floodlight) appendScript(`https://www.googletagmanager.com/gtag/js?id=${encodeURIComponent(ga4 || floodlight)}`);
      ...
      if (tiktok) {
        ...
        ttq._i = ttq._i || {}; ttq._i[tiktok] = []; ttq._t = ttq._t || {}; ttq._t[tiktok] = Date.now();
        appendScript(`https://analytics.tiktok.com/i18n/pixel/events.js?sdkid=${encodeURIComponent(tiktok)}&lib=ttq`);
      }
      sendPageView();
    }
    void start();
  ...
  useEffect(() => {
    if (consent?.analytics && startedAnalytics.current && posthog.__loaded) posthog.capture('$pageview', { $current_url: window.location.href });
...
function sendPageView() {
  try { window.gtag?.('event', 'page_view', { page_path: location.pathname + location.search }); } catch {}
