...
const worker = new Worker('tsk-jobs', async (job) => {
  ...
  if (job.name === 'maintenance.backup') {
    ...
    const response = await fetch(`${origin}/api/cron/maintenance`, { method: 'POST', headers: process.env.CRON_SECRET ? { authorization: `Bearer ${process.env.CRON_SECRET}` } : {} });
