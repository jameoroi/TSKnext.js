  ...
  persist(
    (set) => ({
      ...
      add: (product, qty = 1, variantId = '') =>
        set((state) => {
          ...
          const max = Number(
            variant?.available ?? variant?.stock ?? product.available ?? product.stock ?? 9999,
          );
          ...
          if (existing)
            return {
              items: state.items.map((item) =>
                lineKey(item.id, item.variant_id || '') === key
      ...
      remove: (id, variantId = '') =>
        set((state) => ({
          items: state.items.filter(
            (item) => lineKey(item.id, item.variant_id || '') !== lineKey(id, variantId),
          ),
        })),
      setQty: (id, qty, variantId = '') =>
        set((state) => ({
          items: state.items.map((item) =>
            lineKey(item.id, item.variant_id || '') === lineKey(id, variantId)
    ...
    }),
    {
      ...
      migrate: (persisted: any) => ({
        ...
        bundleClaim:
          persisted?.bundleClaim && typeof persisted.bundleClaim === 'object' ? persisted.bundleClaim : null,
      }),
    },
  ),
);
