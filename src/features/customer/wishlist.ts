...
const unique = (values: unknown[]) =>
  [...new Set(values.map((value) => String(value || '').trim()).filter(Boolean))].slice(0, 200);
...
export function useWishlist() {
    ...
    if (!mergePromise) {
      mergePromise = (async () => {
        ...
        for (const id of guestIds) {
          ...
          try {
            const result = await legacyRequest<{ ids?: string[] }>(
  ...
  const toggle = useCallback(
    async (productOrId: string | { id?: unknown }) => {
      ...
      try {
        const result = await legacyRequest<{ added?: boolean; ids?: string[] }>(
          'customer.wishlist.toggle',
          {
            product_id: id,
            csrf: session.data?.csrf || '',
  ...
  return {
    ...
    toggle,
    reload: () =>
      signedIn ? queryClient.invalidateQueries({ queryKey: ['customer.wishlist'] }) : Promise.resolve(),
  };
}
