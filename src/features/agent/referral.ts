...
export function readAgentRef() {
  ...
  try { return normalizeAgentRef(window.localStorage.getItem(AGENT_REF_KEY)); }
  catch { return ''; }
}
...
export function rememberAgentRef(value: unknown) {
  ...
  if (!code) return;
  try { window.localStorage.setItem(AGENT_REF_KEY, code); }
  catch { /* Private mode/storage policy: the HttpOnly attribution cookie remains authoritative. */ }
}
