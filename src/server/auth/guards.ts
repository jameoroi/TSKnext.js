...
export async function requireRole(role: 'admin' | 'owner' | 'agent' | 'supplier' | 'customer', returnTo = '/') {
  ...
  if (!signedIn) redirect(`/login?role=${role === 'customer' ? '' : role}&redirect=${encodeURIComponent(returnTo)}`);
